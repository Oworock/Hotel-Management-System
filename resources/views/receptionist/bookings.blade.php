...
<div class="animate-fade-in">
    <!-- Filter Panel -->
    <div class="glass-panel" style="padding: 1.5rem; margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: flex-end; flex-wrap: wrap; gap: 1rem;">
        <form action="{{ route('receptionist.bookings') }}" method="GET" style="display: flex; flex-wrap: wrap; gap: 1rem; align-items: flex-end; margin: 0; flex-grow: 1;">
            <div class="form-group" style="flex: 2; min-width: 250px; margin-bottom: 0;">
                <label for="search" class="form-label" style="font-size: 0.85rem; font-weight: 600;">Search Guest Name or Email</label>
                <input type="text" name="search" id="search" class="form-control" placeholder="e.g. Arthur Dent" value="{{ request('search') }}">
            </div>
            ...
            <div style="display: flex; gap: 0.5rem; margin-bottom: 0;">
                ...
                @if(request()->anyFilled(['search', 'status']))
                    <a href="{{ route('receptionist.bookings') }}" class="btn btn-outline" style="padding: 0.6rem 1.5rem; text-decoration: none; display: flex; align-items: center; justify-content: center;">
                        Clear
                    </a>
                @endif
            </div>
        </form>
        <button onclick="toggleBookingModal(true)" class="btn btn-primary" style="padding: 0.6rem 1.5rem; display: inline-flex; align-items: center; gap: 0.5rem;">
            ...
            <table class="custom-table">
                ...
                <tbody>
                    @forelse($bookings as $booking)
                        <tr>
                            <td style="font-family: monospace; font-weight: 600;">#{{ $booking->id }}</td>
                            <td>
                            ...
                            <td style="font-size: 0.85rem; color: var(--text-secondary);">
                                <div>In: {{ $booking->check_in_date->format('M d, Y') }}</div>
                                <div>Out: {{ $booking->check_out_date->format('M d, Y') }}</div>
                            </td>
                            <td>
                                <div style="font-weight: 600;">${{ number_format($booking->total_price, 2) }}</div>
                                <div>
                            ...
                            <td style="text-align: right; white-space: nowrap;">
                                ...
                                @if($booking->payment_status !== 'paid' && in_array($booking->status, ['pending', 'confirmed']))
                                    <form action="{{ route('receptionist.bookings.pay', $booking->id) }}" method="POST" style="display: inline-block; margin: 0 0.25rem;">
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-success" style="font-size: 0.75rem; padding: 0.35rem 0.65rem;">
                                ...
                                @if($booking->status === 'confirmed' && $booking->payment_status === 'paid')
                                    <form action="{{ route('receptionist.bookings.check_in', $booking->id) }}" method="POST" style="display: inline-block; margin: 0 0.25rem;">
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-primary" style="font-size: 0.75rem; padding: 0.35rem 0.65rem;">
                                ...
                                @if($booking->status === 'checked_in')
                                    <form action="{{ route('receptionist.bookings.check_out', $booking->id) }}" method="POST" style="display: inline-block; margin: 0 0.25rem;">
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-outline" style="border-color: var(--warning); color: var(--warning); font-size: 0.75rem; padding: 0.35rem 0.65rem;">
                                ...
                                @if(in_array($booking->status, ['pending', 'confirmed']))
                                    <form action="{{ route('receptionist.bookings.cancel', $booking->id) }}" method="POST" style="display: inline-block; margin: 0 0.25rem;" onsubmit="return confirm('Are you sure you want to cancel this booking?');">
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-outline" style="border-color: var(--danger); color: var(--danger); font-size: 0.75rem; padding: 0.35rem 0.65rem;">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" style="text-align: center; color: var(--text-muted); padding: 3rem 0;">
                                <i class="fa-regular fa-folder-open" style="font-size: 2rem; margin-bottom: 1rem; display: block; opacity: 0.5;"></i>
...
<div id="booking-modal" style="display: none; position: fixed; inset: 0; background: rgba(0, 0, 0, 0.5); z-index: 10000; align-items: center; justify-content: center; backdrop-filter: blur(8px); overflow-y: auto; padding: 2rem 0;">
    <div class="glass-panel" style="max-width: 600px; width: 90%; padding: 2rem; position: relative; margin: auto;">
        ...
        <form action="{{ route('receptionist.bookings.walkin') }}" method="POST" style="margin: 0;">
            @csrf
